    ...
    <>
      <header className="adminPage__head">
        ...
        <p className="adminLede">
          Leave the dates empty when they are not confirmed. The public page then shows &quot;dates
          to be confirmed&quot; — never a guessed date.
        </p>
      </header>

      <div className="adminSplit">
        <section className="adminPanel" aria-label="Edition editor">
        ...
        <section className="adminPanel" aria-label="All editions">
          <h2>
            All editions <span className="adminCount">· {events.length}</span>
          </h2>
          {events.length === 0 ? (
            <div className="adminEmpty">
              No edition has been created yet. Save one on the left to open a salon page.
            </div>
          ) : (
                ...
                <tbody>
                  {events.map((e) => (
                    <tr key={e.id}>
                      ...
                      <td>
                        <span className={`adminChip adminChip--${e.state}`}>{e.state}</span>
                      </td>
                      <td>
