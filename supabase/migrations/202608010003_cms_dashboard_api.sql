...
begin
  ...
  with module_rows as (
    select
      'pages'::text as module_key,
      page.status,
      exists (
        select 1
        from public.site_locales locale
        where locale.site_id = page.site_id
          and locale.enabled
          and not exists (
            select 1
            from public.page_translations translation
            where translation.page_id = page.id
              and translation.site_id = page.site_id
              and translation.locale = locale.locale
          )
      ) as incomplete
    from public.pages page
    where page.site_id = p_site_id and page.deleted_at is null
    ...
    where event.site_id = p_site_id and event.deleted_at is null
    ...
    where asset.site_id = p_site_id and asset.deleted_at is null
  ), modules(module_key) as (
    values ('pages'::text), ('events'::text), ('media'::text)
  )
  select
    module.module_key,
    count(row.module_key)::bigint,
  ...
  from modules module
  left join module_rows row on row.module_key = module.module_key
  group by module.module_key
