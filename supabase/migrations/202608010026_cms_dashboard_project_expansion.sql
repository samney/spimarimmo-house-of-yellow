...
begin
  ...
  with module_rows as (
    ...
    union all
    select 'venues', venue.status,
      cardinality(app_private.venue_warnings_v1(venue.id)) > 0
    from public.venues venue where venue.site_id = p_site_id and venue.deleted_at is null
    union all
    ...
    union all
    select 'projects', project.status,
      cardinality(app_private.project_warnings_v1(project.id)) > 0
    from public.projects project where project.site_id = p_site_id and project.deleted_at is null
    union all
    select 'industries', industry.status,
      cardinality(app_private.industry_warnings_v1(industry.id)) > 0
    from public.industries industry where industry.site_id = p_site_id and industry.deleted_at is null
    union all
    select 'project_categories', category.status,
      cardinality(app_private.project_category_warnings_v1(category.id)) > 0
    from public.project_categories category where category.site_id = p_site_id and category.deleted_at is null
    union all
    select 'project_tags', tag.status,
      cardinality(app_private.project_tag_warnings_v1(tag.id)) > 0
    from public.project_tags tag where tag.site_id = p_site_id and tag.deleted_at is null
    union all
    ...
    union all
    select 'faqs', faq.status, cardinality(app_private.faq_warnings_v1(faq.id)) > 0
    from public.faqs faq where faq.site_id = p_site_id and faq.deleted_at is null
  ), modules(module_key) as (
    values ('pages'::text), ('events'::text), ('venues'::text), ('media'::text),
      ('projects'::text), ('industries'::text), ('project_categories'::text),
      ('project_tags'::text), ('exhibitor_packages'::text),
      ('content_partners'::text), ('case_studies'::text), ('metrics'::text),
      ('resources'::text), ('testimonials'::text), ('articles'::text), ('faqs'::text)
  )
