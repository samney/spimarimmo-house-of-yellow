...
export class PostgresContentRepository implements ContentRepository {
  ...
  private async queryEvents(
    ...
    const rows = await this.sql.query<{
      ...
      `select e.id, e.slug, t.name, t.short_description,
    ...
    return rows.map((r) => ({
      ...
      locale,
      name: r.name,
      // The schema's editor-facing blurb. Empty string when unwritten, which is
      // what the seam documents a card must survive.
      summary: r.short_description,
      // Approved media reaches events through the asset-link tables, which this
      // adapter does not join yet. `null` is the honest answer the seam asks
      // for; a borrowed photo on a public card would not be.
      image: null,
