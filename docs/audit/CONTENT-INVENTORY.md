...
- **Consent**: Complianz opt-in banner with Accept / Deny buttons and **in-banner category toggles** (functional, preferences, statistics, marketing — `cmplz-category-toggle` checkboxes; there is NO separate preferences dialog on the banner, verified 2026-07-30 by selector probing). Ongoing preference management lives in the embedded widget on `/cookies/` (`#cmplz-manage-consent-container`). Strings include EN + NL; z-index 99999/100000.
