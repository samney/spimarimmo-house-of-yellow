...
- [~] **F-07 · Foundation regression tests.** → `motion-foundation.test.ts`, 8 guards: tokens mirror the CSS ladder; no `opacity: 0` rule is gated on a class nothing produces; no primitive ships unmounted; every `infinite` animation states its own reduced-motion rest state; no primitive hard-codes a duration, ease or trigger point. The reduced-motion guard found the `.promoProgressLine` defect on its first run, and the vocabulary guard found Lenis's bare `1.2` on its first run. The orphan guard was written wrong first and **passed vacuously**, substring-matching a word inside an unrelated comment while two primitives were mounted nowhere; it now matches the import path and asserts set _equality_ so the allowlist cannot rot. Remaining: a runtime reduced-motion spec — currently verified in-browser per slice, not pinned.
