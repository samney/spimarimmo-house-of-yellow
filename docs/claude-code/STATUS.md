...
- `SPI-010`: `IN PROGRESS` at `P1.1`. `TRF-002` residue inventory is complete on `claude/spi-010-trf-002-residue-inventory`; record in `docs/spimar-phase-1/RESIDUE-INVENTORY.md`. 29 tracked source files carry residue in 259 occurrences, split into ~93 visible (brand, copy, contact, legal) and ~169 internal identifiers. New blocker `LEG-1`: the `/cookies` policy is a third party's legal text and cannot be corrected by editing.
