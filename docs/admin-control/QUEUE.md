...
| ADM-072 | My Day                            | done     | "Ma journée" on /admin/tasks: overdue / 7-day / newly-assigned from the audit trail; appointments stated as not connected (ADM-082 deferred); `crm-my-day.spec.ts` 1/1                              |
...
| ADM-087 | Lost reason                       | done     | Lead.lostReason + closed vocabulary; required at the action boundary, cleared by the adapter on reopen (probe-verified); the board hands lost moves to the workspace; `crm-lost-reason.spec.ts` 2/2 |
