...
9. `docs/claude-code/SPIMAR-TRACEABILITY.md`
10. `docs/migration/MIGRATION-COVERAGE.md`
11. Every file in `docs/SPIMAR-Transformation-Phase-1/`, in numeric order, before any Phase 1 edit
12. Specification files linked by the active queue item

Update `STATUS.md`, `QUEUE.md`, `VALIDATION-MATRIX.md`, `SPIMAR-TRACEABILITY.md`, and `SESSION-HANDOFF.md` before ending a work session.

There is no `AGENTS.md` in this repository. Where the Phase 1 package instructs Claude to read `AGENTS.md`, read this file and `.claude/rules/`.
...
4. `docs/SPIMAR-Transformation-Phase-1/` — controlling Phase 1 execution contract (`D-016`)
5. Canonical files under `docs/spimar/official-specifications/` — product/UX/content corpus and the authority for anything Phase 1 does not cover
6. Current Git history, tests, rendered application and accepted evidence
7. Supporting audits and evidence
8. Superseded/rejected/archive material
...
- Active item: `TRF-002` — SPIMAR Transformation Phase 1. Canonical Phase 1 contract is `docs/SPIMAR-Transformation-Phase-1/` (adopted by `D-016`); canonical backlog is `TRF-000`–`TRF-090`. The retired `SPI-*` scheme must not be used. Do not continue polishing the House of Yellow clone.
